...
const WEBHOOK_URL = getEnv(
  'WEBHOOK_URL',
  'https://api.demo.wonderful.ai/api/v1/tasks/webhook/46c154f5-10c9-462a-9d5a-882a662c32fe'
);
const WEBHOOK_SECRET = getEnv(
  'WEBHOOK_SECRET',
  '250c8ded-cd8a-4bdc-8029-7709ca6bbc8c'
);
...
const uploadAttachment = async (file) => {
  ...
  const filename = file.name || 'provident-fund-application-upload';
...
const triggerWebhook = async ({ email, firstName, lastName, attachmentId }) => {
  const subject = `בקשת הצטרפות לקופת גמל - ${firstName} ${lastName}`.trim();
  ...
  const webhookResponse = await fetch(WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      ...
      payload: {
        ...
        subject,
        message: 'Process this provident fund application',
...
server.listen(PORT, () => {
  console.log(`Provident fund application UI server running at http://localhost:${PORT}`);
});
